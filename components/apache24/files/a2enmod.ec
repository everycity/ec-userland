...
if ( $obj eq 'mod' ) {
    ...
    $dir    = 'mods';
    $sffx   = '.load';
    $reload = 'svcadm disable -s svc:/network/http/apache22 && svcadm enable svc:/network/http/apache22';
}
else {
    $dir    = 'sites';
    $sffx   = '';
    $reload = 'svcadm restart svc:/network/http/apache22';
}
$name = ucfirst($obj);

my $confdir  = $ENV{APACHE_CONFDIR}                  || "/ec/etc/apache/2.4";
...
info("Run '$reload' to activate new configuration!\n")
    if $request_reload;
