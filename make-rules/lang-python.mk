...
PYTHON_VENDOR_PACKAGES= $(PYTHON_PREFIX)/lib/python$(PYTHON_VERSION)/vendor-packages
...
$(BUILD_DIR)/%/.built:	$(SOURCE_DIR)/.prep $(BUILD_DIR)/config-%/$(CFG)
	...
	(cd $(SOURCE_DIR) ; $(ENV) HOME=$(BUILD_DIR)/config-$* $(PYTHON_ENV) \
		$(PYTHON.$(BITS)) ./setup.py build $(PYTHON_BUILD_ARGS))
...
GENERATE_TRANSFORMS+=    $(WS_TRANSFORMS)/generate-python-cleanup
PKG_MACROS+=	PYTHON_PREFIX=$(shell echo $(PYTHON_PREFIX) | sed 's;^/;;')
PKG_MACROS+=	PYTHON_VERSION=$(shell echo $(PYTHON_VERSION) | sed 's;^/;;')
